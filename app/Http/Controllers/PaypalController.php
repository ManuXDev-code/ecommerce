...
class PaypalController extends Controller
{
    ...
    public function gracias(Request $request)
    {
        ...
        // Lógica para la página de agradecimiento después del pago exitoso
        $token = $request->query('token');
        try {
            $response = $this->provider->capturePaymentOrder($token);

            if (isset($response['status']) && $response['status'] == 'COMPLETED') {
                // Aquí puedes actualizar el estado del pedido en tu base de datos si es necesario
    ...
    {
        // Lógica para la página de orden completada
        return view('web.orden_completado');
    }

    public function cancelar()
    {
        // Lógica para la página de cancelación del pago
    }
}
